...
bike_df = pd.read_csv(file_path)
    ...
    elif "cnt_x" in all_df.columns:
        cnt_column = "cnt_x"
    elif "cnt_y" in all_df.columns:
        cnt_column = "cnt_y"
    else:
...
with col1:
    total_orders = all_df[cnt_column].sum()
    st.metric("Total Penyewaan", value=total_orders)
with col2:
    total_revenue = format_currency(total_orders * 5, "USD", locale='en_US')  # Asumsi rata-rata harga sewa $5
    st.metric("Total Revenue", value=total_revenue)

# Grafik Penyewaan Harian
st.subheader('📅 Penyewaan Harian')
fig = px.line(all_df, x="dteday", y=cnt_column, markers=True, title="Tren Penyewaan Harian")

# Penyewaan Berdasarkan Musim
st.subheader("🌦️ Penyewaan Berdasarkan Musim")
byseason_df = all_df.groupby("season").agg({cnt_column: "sum"}).reset_index()
...
workday_df = all_df.groupby("workingday").agg({cnt_column: "sum"}).reset_index()
...
hourly_df = all_df.groupby("hr").agg({cnt_column: "sum"}).reset_index()
